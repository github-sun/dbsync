...
public class AdminDAOImpl implements AdminDAO {
	...
	public boolean addAdmin(Admin admin) {
		logger.error("==addAdmin " + admin.toString());
		SqlSession session = MyBatisUtil.getSession(Common.outerConfigFile);
		try {
			int result = session.insert(namespace + "addAdmin", admin);
			// 注意：此处有陷阱，如果做了更新、插入或删除操作，必须使用：
			session.commit();
